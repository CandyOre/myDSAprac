...
namespace mySTL {
    ...
    private:
        int _size;
        ListNodePosi(T) header; ListNodePosi(T) trailer;
    ...
    void List<T>::init () {
        ...
        header->suc = trailer; header->pre = nullptr;
        trailer->pre = header; trailer->suc = nullptr;
        _size = 0;
    }
    ...
    void List<T>::selectionSort ( ListNodePosi(T) p, int n ) {
        ListNodePosi(T) head = p->pre; ListNodePosi(T) tail = p;
    ...
    ListNodePosi(T) List<T>::selectMax ( ListNodePosi(T) p, int n ) const {
        ...
        for ( ListNodePosi(T) cur = p; 1 < n; n-- )
            if ( !lt ( ( cur = cur->suc )->data, max->data ) )
                max = cur;
        return max;
    }
